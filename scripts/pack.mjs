...
for (const directory of ['cocos', 'cocos-session-replay']) {
  const metadata = JSON.parse(await readFile(`packages/${directory}/package.json`, 'utf8'));
  const packResult = spawnSync(
    process.platform === 'win32' ? 'npm.cmd' : 'npm',
    ['pack', '--workspace', metadata.name, '--pack-destination', 'artifacts', '--json'],
    { encoding: 'utf8', env: { ...process.env, npm_config_cache: process.env.npm_config_cache || '/tmp/cocos-sdk-npm-cache' } },
  );
  ...
  const zipPath = resolve('artifacts', `truewatch-${directory}-${metadata.version}.zip`);
  try {
    ...
    // Use the npm whitelist for ZIP downloads as well.
    await rm(zipPath, { force: true });
    const zipped = spawnSync('zip', ['-qr', zipPath, '.'], { cwd: join(staging, 'package'), stdio: 'inherit' });
